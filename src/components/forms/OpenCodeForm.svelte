<script lang="ts">
  import type { AgentLaunch, OpenCodeResumeMode, SessionDraft } from '@shared/types/sessions.js';
  ...
  const resumeModes: { value: OpenCodeResumeMode; label: string }[] = [
...
<div class="flex flex-col gap-1.5">
  ...
  <Select.Root
    ...
    onValueChange={(value) => update('resumeMode', value as OpenCodeResumeMode)}
  >
...
{#if draft.resumeMode === 'resume_by_id'}
  <div class="flex flex-col gap-1.5">
    <Label class="text-xs text-muted-foreground" for="opencode-session-id">OpenCode session id</Label>
    <Input
      id="opencode-session-id"
      value={draft.openCodeSessionId ?? ''}
      oninput={(event) => update('openCodeSessionId', event.currentTarget.value)}
    />
  </div>
{/if}
