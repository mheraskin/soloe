...
// @ts-expect-error -- this Vitest-only ABI check reads the vendored binary from disk.
import { readFileSync } from "node:fs";

import { ghosttyKeyForCode } from "./keyCodes";

const wasmDataUrl = new Uint8Array(
  readFileSync(new URL("./vendor/ghostty-vt.wasm", import.meta.url)),
);
const writePtyWasmDataUrl = new Uint8Array(
  readFileSync(new URL("./vendor/ghostty-write-pty.wasm", import.meta.url)),
);
const pinnedVersion = readFileSync(new URL("./vendor/VERSION", import.meta.url), "utf8");

type WasmFunction = (...args: number[]) => number;

function decodeWasmDataUrl(dataUrl: Uint8Array): Uint8Array {
  return dataUrl;
}
