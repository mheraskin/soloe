...
/**
 * Retains a bounded, sequence-aware tail for live terminals. The Interface
 * exposes one immutable snapshot while per-terminal/global eviction and byte
 * accounting stay local to this Module.
 */
export class TerminalReplayBuffer {
  private readonly states = new Map<TerminalId, ReplayState>();
  // Both chronologies contain live chunks only. Set preserves insertion order
  // while deleting an exact evicted chunk in amortized O(1), so an unrelated
  // quiet terminal cannot pin a noisy terminal's discarded payload.
  ...
  append(event: TerminalOutputEvent): void {
    ...
    while (
      ...
      const oldest = this.nextGlobalChunk();
      if (!oldest) break;
      const owner = this.states.get(oldest.terminalId);
      if (owner) {
        this.evictChunk(owner, oldest);
      } else {
        // Structural repair: remove an impossible orphan rather than letting
        // one corrupt entry stall global admission forever.
  ...
  snapshot(terminalId: TerminalId, afterSeq = 0): TerminalReplaySnapshot | null {
    ...
    const truncated = requestedAfter === 0
      ? state.truncated
  ...
  private nextGlobalChunk(): ReplayChunk | null {
    return firstOf(this.globalOrder);
  }
}
