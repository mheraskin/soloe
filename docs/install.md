...
- `wl-clipboard` for native Claude Code image paste on Wayland Linux;
...
pnpm setup:linux # Ubuntu only; installs wl-clipboard and configures Electron's sandbox
pnpm dev
```
...
The first validated release is intended to include one signed and notarized Soloe DMG for each macOS architecture, an unsigned Windows installer, and Linux AppImage/DEB artifacts with SHA-256 checksums. The Debian package declares `wl-clipboard` as a dependency; AppImage users on Wayland must install it separately. Verify the checksum before running a downloaded artifact. Unsigned Windows builds can trigger SmartScreen.
