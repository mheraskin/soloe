...
export class SessionTranscriptReader {
  ...
  constructor(opts: SessionTranscriptReaderOptions = {}) {
    ...
    this.useWslHostBridge = opts.useWslHostBridge ?? process.platform === 'win32';
  }
  ...
  private async peekScopedFile(
    ...
    const translated = this.translateRendererPath(rendererPath, scope);
    const file = await this.authorizeTranscriptPath(translated, scope);
    if (!file) return null;
    if (isClaudeTranscriptPath(rendererPath)) {
      return this.peekClaudeFile(file, cwd);
    }
  ...
  private translateRendererPath(p: string, scope?: SessionScope): string {
    if (scope?.runMode !== 'wsl' || !this.useWslHostBridge) return p;
    if (!p.startsWith('/')) return p;
    return posixToWslUnc(scope.wslDistro ?? 'Ubuntu', p);
  }

  private async authorizeTranscriptPath(
    file: string,
    scope?: SessionScope
  ): Promise<string | null> {
    if (!path.isAbsolute(file) && !path.win32.isAbsolute(file)) return null;
    const dirs = await this.resolveDirs(scope);
    const canonicalFile = await safeRealpath(file);
    if (!canonicalFile) return null;
    for (const root of [dirs.claudeProjectsDir, dirs.codexSessionsDir]) {
      const canonicalRoot = (await safeRealpath(root)) ?? pathFor(root).resolve(root);
      if (isWithin(canonicalRoot, canonicalFile)) return canonicalFile;
    }
    return null;
  }
  ...
  private async peekClaudeFile(
    ...
    let lastUuid = '';
    let transcriptCwd: string | undefined;
    for (const line of lines) {
      const rec = parseJsonLine(line);
      if (!rec) continue;
      if (typeof rec['cwd'] === 'string' && !transcriptCwd) {
        transcriptCwd = rec['cwd'] as string;
      }
    ...
    if (expectedCwd && (!transcriptCwd || !pathsEqual(transcriptCwd, expectedCwd))) {
      return null;
    }
    return {
...
function pathFor(value: string): typeof path.posix | typeof path.win32 {
