...
export interface OverviewExecutionScope {
  cwd: string;
  runMode: RunMode;
  wslDistro?: string;
}

export interface OverviewAgentExecution {
  execute(request: {
    candidates: ModelSelection[];
    binaries: Settings['binaries'];
    scope: OverviewExecutionScope;
    prompt: string;
    timeoutMs: number;
...
export class WorktreeOverviewService {
  ...
  private readonly execution: OverviewAgentExecution;
  ...
  private executionScope(scope: OverviewExecutionScope): OverviewExecutionScope {
    return this.opts.resolveExecutionScope?.(scope) ?? scope;
  }
