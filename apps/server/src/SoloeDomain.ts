...
export class SoloeDomain extends EventEmitter {
  ...
  async invoke(call: DomainCall): Promise<unknown> {
    ...
    throw new Error(`Unsupported browser RPC: ${call.namespace}.${call.method}`);
  }
  ...
  private async projectsCall(method: string, args: unknown[]): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`Unsupported browser RPC: projects.${method}`);
    }
  }
  ...
  private async sessionsCall(method: string, args: unknown[]): Promise<unknown> {
    switch (method) {
      ...
      case "create":
        return this.changeSession(await this.sessions.create({
      ...
      default:
        throw new Error(`Unsupported browser RPC: sessions.${method}`);
    }
  }
  ...
  private async terminalCall(method: string, args: unknown[]): Promise<unknown> {
    switch (method) {
      ...
      case "setOutputDemand":
        return true;
      default:
        throw new Error(`Unsupported browser RPC: terminal.${method}`);
    }
  }
